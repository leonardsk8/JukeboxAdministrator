         ...
         switch(val){
             ...
             case 2:
                 ...
                 for (SongVO s:listToApproved){
                     int x = (s.getName().length()>37)?37:s.getName().length();
                     var += ""
                         ...
                         + "','"+String.valueOf(request.getSession().getAttribute("UID"))+"','"+s.getThumbnail()+"','"+s.getUser()+"','"+s.getToken()
                         +"','"+s.getUserId()+"',true)\""
                         + "'>Aprobar</button><button type=\"button\" class=\"btn btn-danger btn1A\""
                                 + "onclick=\"removeSong('"+s.getVideo_id()+"',"+true+",'"+s.getToken()+"','"+s.getName()+"')\">Rechazar</button></li>"
                         + "\n"+s.getUser();
                 }
